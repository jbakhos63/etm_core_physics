...
A_RELEASE_TICK = 50  # Identity A will be removed after this tick

# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_153_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial153.json")
...
# Two identities with same ancestry/phase
identities = [
    {
        'x': 6.5,
        'y': 1.0,
...
# Lock memory and exclusion logic
modular_locks = {}
lock_ticks = {}
...
for tick in range(TICK_LIMIT):
    for identity in identities:
        ...
        signature = (identity['ancestry'], round(identity['phase'], 2))

        # Remove A's lock after a certain tick
        if tick == A_RELEASE_TICK and 'A' in lock_ticks:
            modular_locks.pop(signature, None)

        # Locking logic
        if identity['id'] not in lock_ticks and support >= LOCK_THRESHOLD:
            if signature not in modular_locks:
                for r in recruiters:
                    if phase_distance(identity['phase'], r['phase']) <= LOCK_TOLERANCE:
                        modular_locks[signature] = identity['id']
                        lock_ticks[identity['id']] = tick
                        break
            else:
                excluded.append({'id': identity['id'], 'tick': tick})
