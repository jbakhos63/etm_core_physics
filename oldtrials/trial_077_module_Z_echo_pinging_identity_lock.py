...
# trial_077_module_Z_echo_pinging_identity_lock.py

"""
Trial 077: Echo-Pinging Identities and Recruiter Phase Lock-In

Goal:
- Identities emit ancestry-tagged echo pulses every few ticks
- Recruiters track ancestry and adjust their phase toward recent echo input
- If memory + phase quorum sustained for 20 ticks, recruiters freeze phase (lock)
- Intruder tests entry after lock
"""
...
# Recruiter with echo memory and phase adaptation
class EchoPingingRecruiter(RecruiterNode):
...
for t in range(TOTAL_TICKS):
    ...
    # Emit echo pulses every ECHO_INTERVAL ticks
    if (t - DROP_TICK) % ECHO_INTERVAL == 0 and t >= DROP_TICK:
        for r in recruiters.values():
            r.receive_echo(ANCESTRY_P, identity_P.phase % 1.0, REINFORCEMENT_BASE)
            r.receive_echo(ANCESTRY_N, identity_N.phase % 1.0, REINFORCEMENT_BASE)
...
with open(os.path.join(output_dir, "trial_077_summary.json"), "w") as f:
    json.dump({
        "trial": "077",
        "drop_tick": DROP_TICK,
...
with open(os.path.join(output_dir, "transition_log_trial077.json"), "w") as f:
    json.dump(tick_log, f, indent=2)

print("✓ Trial 077 complete. Results written to '../results/' folder.")
