...
# trial_063_module_Z_phase_mismatch_sweep.py

"""
Trial 063: Modular Reentry Phase Mismatch Sweep

Goal:
- Introduce five identities at tick 90 with phase offsets:
  +0.001, +0.01, +0.05, +0.10, +0.12
- Test which can phase-lock into an already drifting recruiter rhythm
- Map ETM's rhythm coherence envelope
"""
...
TOTAL_TICKS = 100
PHASE_DRIFT = 0.0005
OFFSETS = [0.001, 0.01, 0.05, 0.10, 0.12]
...
# Return identities with phase offsets
return_identities = {}
...
for t in range(TOTAL_TICKS):
    ...
    # Reintroduce each test identity with phase offset
    if t == REJOIN_TICK:
...
with open(os.path.join(output_dir, "trial_063_summary.json"), "w") as f:
    json.dump({
        "trial": "063",
        "drop_tick": DROP_TICK,
...
with open(os.path.join(output_dir, "transition_log_trial063.json"), "w") as f:
    json.dump(tick_log, f, indent=2)

print("✓ Trial 063 complete. Results written to '../results/' folder.")
