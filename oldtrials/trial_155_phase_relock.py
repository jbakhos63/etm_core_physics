...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_155_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial155.json")
...
for tick in range(TICK_LIMIT):
    ...
    for r in recruiters:
        ...
        reinforcement = max(0.0, 1.0 - REINFORCEMENT_DECAY * dist - base_phase_diff)
        if tick == RELOCK_TICK:
    ...
    # Re-locking logic
    if not identity['locked'] and not identity['relocked'] and tick > unlock_tick and support >= LOCK_THRESHOLD:
        for r in recruiters:
            if phase_distance(identity['phase'], r['phase']) <= LOCK_TOLERANCE:
                identity['locked'] = True
