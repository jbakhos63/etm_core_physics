...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_121_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial121.json")
...
# Left wall: phase 0.25, fixed along x = -3
for y in range(-2, 3):
    recruiters.append({'x': -3.0, 'y': float(y), 'phase': 0.25})

# Right wall: phase 0.75, fixed along x = +3
for y in range(-2, 3):
    recruiters.append({'x': 3.0, 'y': float(y), 'phase': 0.75})
