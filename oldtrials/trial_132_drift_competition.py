...
PHASE_DRIFT_RATE = 0.0005  # incremental drift added per tick to the right cluster

# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_132_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial132.json")
...
# Left recruiter cluster (fixed phases near 0.48)
left_cluster = [
...
# Right recruiter cluster (initial phases near 0.52, with drift over time)
initial_right_cluster = [
...
for tick in range(TICK_LIMIT):
    ...
    drifted_right_cluster = []
    for r in initial_right_cluster:
        drifted = r.copy()
        drifted['phase'] = (r['phase'] + PHASE_DRIFT_RATE * tick) % 1.0
        drifted_right_cluster.append(drifted)

    recruiters = left_cluster + drifted_right_cluster
