...
# Identity definitions (same ancestry, same spin)
identity_A = {"id": "A", "ancestry": "rotorX", "spin": "up", "phase": 0.0, "position": 9.0}
identity_B = {"id": "B", "ancestry": "rotorX", "spin": "up", "phase": 0.0, "position": 15.0}

# Shared recruiter field
recruiter_centers = [10, 12, 14]

# Recruiter memory: stores locked identity tags
recruiter_memory = {center: set() for center in recruiter_centers}

locks = {"A": [], "B": []}

def identity_tag(identity):
    return f"{identity['ancestry']}_{identity['spin']}"
...
for t in range(ticks):
    for identity in [identity_A, identity_B]:
        ...
        for center in recruiter_centers:
            ...
            if tag in recruiter_memory[center]:
                memory_conflict = True  # identity conflict in recruiter memory

            if phase_diff < phase_tolerance:
                support += 1

        if support >= support_threshold and not memory_conflict:
            locks[identity["id"]].append(t)
            for center in recruiter_centers:
                recruiter_memory[center].add(tag)

        identity["position"] += 0.032
...
with open("../results/trial_200_recruiter_exclusion_summary.json", "w") as f:
