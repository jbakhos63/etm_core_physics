...
BUFFER_REINFORCEMENT = 0.6

# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_159_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial159.json")

def phase_distance(a, b):
    return min(abs(a - b), 1 - abs(a - b))

# Recruiters (baseline G-phase ring)
base_recruiters = []
...
# Temporal ancestry buffer (simulates recruiter echo memory)
buffer_active = False

# Simulation loop
for tick in range(TICK_LIMIT):
    # Maintain stable rhythm until snap window
    phase_increment = PHASE_INCREMENT_START + (PHASE_INCREMENT_DRIFT if tick >= DRIFT_TICK and tick < BUFFER_START else 0.0)
    ...
    if BUFFER_START <= tick < BUFFER_START + BUFFER_DURATION:
        buffer_active = True
        for i in range(RECRUITER_COUNT):
            angle = 2 * math.pi * i / RECRUITER_COUNT
            x = RECRUITER_RING_RADIUS * math.cos(angle)
            y = RECRUITER_RING_RADIUS * math.sin(angle)
            recruiters.append({'x': x, 'y': y, 'phase': RECRUITER_PHASE_CENTER})
    ...
    for r in recruiters:
        ...
        if buffer_active and r['phase'] == RECRUITER_PHASE_CENTER:
            reinforcement += BUFFER_REINFORCEMENT
