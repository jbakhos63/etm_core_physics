...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_177_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial177.json")

def phase_distance(a, b):
    return min(abs(a - b), 1 - abs(a - b))

# Two identities, same ancestry, but opposite spin tags
identities = [
    {'id': 'A', 'x': REFLECT_LEFT, 'y': 4.0, 'phase': 0.5, 'vx': 0.3, 'ancestry': 'common', 'spin': 'up', 'locked_ticks': [], 'support_trace': []},
    {'id': 'B', 'x': REFLECT_RIGHT, 'y': 4.0, 'phase': 0.5, 'vx': -0.3, 'ancestry': 'common', 'spin': 'down', 'locked_ticks': [], 'support_trace': []}
]

modular_locks = set()  # Enforce ancestry-phase-spin distinction
transition_log = []
