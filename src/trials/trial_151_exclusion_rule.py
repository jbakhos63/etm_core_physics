...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_151_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial151.json")
...
# Two identities with same ancestry, phase, and descent path
identities = [
    {
        'x': 6.5,
        'y': 1.0,
...
# Exclusion memory: tracks if a prior identity with same ancestry/phase already locked
modular_locks = set()
...
for tick in range(TICK_LIMIT):
    for identity in identities:
        ...
        if identity['id'] not in lock_ticks and support >= LOCK_THRESHOLD:
            # Enforce ancestry-phase exclusion
