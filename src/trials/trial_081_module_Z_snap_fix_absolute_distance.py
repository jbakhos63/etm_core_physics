...
# trial_081_module_Z_snap_fix_absolute_distance.py

"""
Trial 081: Snap Logic Redefined via Absolute-Min Wraparound Distance

Goal:
- Fix snap-to-phase logic using absolute distance with wraparound handling
- Recruiters drift upward and receive echo input with offset
- If recruiter phase enters snap range (±0.03), lock immediately
- Lock-in triggered when phase + memory quorum are met for 20 ticks
"""
...
# Snap recruiter with corrected wraparound check
class SnapFixRecruiter(RecruiterNode):
    ...
    def adjust_phase(self):
        if self.locked or not self.is_memory_ready() or self.recent_echo is None:
            return
        # Snap using absolute min wraparound distance
...
with open(os.path.join(output_dir, "trial_081_summary.json"), "w") as f:
    json.dump({
        "trial": "081",
        "drop_tick": DROP_TICK,
...
with open(os.path.join(output_dir, "transition_log_trial081.json"), "w") as f:
    json.dump(tick_log, f, indent=2)

print("✓ Trial 081 complete. Results written to '../results/' folder.")
