...
# trial_061_module_Z_dual_return_test.py

"""
Trial 061: Dual Identity Reentry with Mismatched Ancestry

Goal:
- After modular recruiter rhythm is established and identities removed:
  - Reintroduce one proton at tick 80
  - Reintroduce one neutron at tick 85
- Observe whether both identities phase-lock to the drifted recruiter rhythm
- Determine whether ancestry mismatch affects reentry compatibility
"""
...
# Initial identities (for rhythm seeding)
...
for t in range(TOTAL_TICKS):
    ...
    # Tick original identities until removal
    if t < REMOVE_TICK:
...
with open(os.path.join(output_dir, "trial_061_summary.json"), "w") as f:
    json.dump({
        "trial": "061",
        "drop_tick": DROP_TICK,
        ...
        "return_identities": ["identity_P_return", "identity_N_return"],
        "ancestry": {
            "identity_P_return": ANCESTRY_P,
            "identity_N_return": ANCESTRY_N
        },
...
with open(os.path.join(output_dir, "transition_log_trial061.json"), "w") as f:
    json.dump(tick_log, f, indent=2)

print("✓ Trial 061 complete. Results written to '../results/' folder.")
