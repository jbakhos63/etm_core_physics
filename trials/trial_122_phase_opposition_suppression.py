...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_122_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial122.json")
...
# Symmetric recruiter setup: phase pairs (0.3 left, 0.7 right), centered on identity phase = 0.5
recruiters = []
phases_left = [0.3] * 5
phases_right = [0.7] * 5
...
for i, y in enumerate(y_positions):
    recruiters.append({'x': -3.0, 'y': float(y), 'phase': phases_left[i]})

# Right wall at x = +3
for i, y in enumerate(y_positions):
    recruiters.append({'x': 3.0, 'y': float(y), 'phase': phases_right[i]})
