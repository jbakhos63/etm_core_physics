...
PHOTON_PHASE_OFFSET = 0.1  # mimic excitation to a higher rhythm module (E1)

# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_147_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial147.json")
...
for tick in range(TICK_LIMIT):
    ...
    for r in recruiters:
        ...
        base_phase_diff = phase_distance(identity['phase'], r['phase'])

        # Check for photon echo contribution
        photon_phase = (r['phase'] + PHOTON_PHASE_OFFSET) % 1.0 if tick in PHOTON_TICKS else r['phase']
        phase_diff = phase_distance(identity['phase'], photon_phase)
