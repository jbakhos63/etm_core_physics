...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_176_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial176.json")

def phase_distance(a, b):
    return min(abs(a - b), 1 - abs(a - b))

# Identities A and B — same ancestry, same tick phase
identities = [
    {'id': 'A', 'x': REFLECT_LEFT, 'y': 4.0, 'phase': 0.5, 'vx': 0.3, 'ancestry': 'common', 'locked_ticks': [], 'support_trace': []},
    {'id': 'B', 'x': REFLECT_RIGHT, 'y': 4.0, 'phase': 0.5, 'vx': -0.3, 'ancestry': 'common', 'locked_ticks': [], 'support_trace': []}
]

modular_locks = set()  # Ancestry + phase exclusion
transition_log = []
...
for tick in range(TICK_LIMIT):
    for identity in identities:
        ...
        support = 0.0
        ancestry_key = (identity['ancestry'], round(current_phase, 3))

        # Recruiter wall match
        ...
        locked = False
        if support >= LOCK_THRESHOLD and ancestry_key not in modular_locks:
            modular_locks.add(ancestry_key)
