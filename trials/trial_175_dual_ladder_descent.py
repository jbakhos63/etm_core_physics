...
# Constants
TICK_LIMIT = 180
PHASE_INCREMENT_ID_A = 0.0125
PHASE_INCREMENT_ID_B = 0.0125
...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_175_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial175.json")
...
identities = [
    {'id': 'A', 'x': REFLECT_LEFT, 'y': 4.0, 'phase': 0.5, 'vx': 0.3, 'locked_ticks': [], 'support_trace': []},
    {'id': 'B', 'x': REFLECT_RIGHT, 'y': 4.0, 'phase': 0.5, 'vx': -0.3, 'locked_ticks': [], 'support_trace': []}
]
...
for tick in range(TICK_LIMIT):
    for identity in identities:
        # Phase evolution
        if identity['id'] == 'A':
            current_phase = (identity['phase'] + PHASE_INCREMENT_ID_A * tick) % 1.0
        else:
            current_phase = (identity['phase'] + PHASE_INCREMENT_ID_B * tick) % 1.0

        support = 0.0

        # Recruiter wall baseline
