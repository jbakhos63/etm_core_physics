...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_178_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial178.json")

def phase_distance(a, b):
    return min(abs(a - b), 1 - abs(a - b))

# Two identities, same ancestry and spin, but phase-staggered
identities = [
    {'id': 'A', 'x': REFLECT_LEFT, 'y': 4.0, 'phase': 0.5, 'vx': 0.3, 'ancestry': 'common', 'spin': 'up', 'locked_ticks': [], 'support_trace': []},
    {'id': 'B', 'x': REFLECT_RIGHT, 'y': 4.0, 'phase': 0.45, 'vx': -0.3, 'ancestry': 'common', 'spin': 'up', 'locked_ticks': [], 'support_trace': []}
]
