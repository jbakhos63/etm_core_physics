...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_119_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial119.json")
...
# Recruiters: two arrays
recruiters = []

# Left recruiter array: phase increases left to center (0.0 to 0.1)
for i in range(6):
    x = -5.0 + i * 1.0
    phase = (0.0 + i * 0.02) % 1.0
    recruiters.append({'x': x, 'y': 0.0, 'phase': phase})

# Right recruiter array: phase decreases right to center (1.0 to 0.9)
for i in range(6):
    x = 1.0 + i * 1.0
