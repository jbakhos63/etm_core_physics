...
DRIFT_TICK = 50
SNAP_TICK = 125
SNAP_PHASE = 0.5  # Phase match target
...
# Output paths
RESULTS_DIR = "../results"
SUMMARY_FILENAME = os.path.join(RESULTS_DIR, "trial_158_summary.json")
TRANSITION_LOG_FILENAME = os.path.join(RESULTS_DIR, "transition_log_trial158.json")
...
for tick in range(TICK_LIMIT):
    # Maintain steady phase to allow alignment with snap recruiter
    phase_increment = PHASE_INCREMENT_START + (PHASE_INCREMENT_DRIFT if tick >= DRIFT_TICK and tick < SNAP_TICK else 0.0)
    ...
    # Inject snap-to recruiter ring at the target phase match
    if tick == SNAP_TICK:
        for i in range(RECRUITER_COUNT):
            angle = 2 * math.pi * i / RECRUITER_COUNT
            x = RECRUITER_RING_RADIUS * math.cos(angle)
            y = RECRUITER_RING_RADIUS * math.sin(angle)
            recruiters.append({'x': x, 'y': y, 'phase': SNAP_PHASE})
    ...
    for r in recruiters:
        ...
        if tick == SNAP_TICK and r['phase'] == SNAP_PHASE:
